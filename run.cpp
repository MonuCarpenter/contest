...
void run_tests(const fs::path &folder) {
  ...
  compile_command +=
      "-o " + folder.string() + "/code " + folder.string() + "/code.cpp";

  std::cout << "\033[1mCompiling...\033[0m\n";
  int compile_status = std::system(compile_command.c_str());

  if (compile_status != 0) {
    std::cerr << "\033[1;31mCompilation failed for folder " << folder
              << ". Exiting.\033[0m\n";
    return;
  }
...
std::cout << "\033[1m(Use OJ=1 ./run folder to simulate judge "
             "environment)\033[0m\n\n";
...
std::cout << "\033[1m=========================================================="
             "======================================================\033[0m\n";
std::cout
    << "\033[1m| Test Case | Status   | Input                | Output          "
       "     | Expected             | Diff                |\033[0m\n";
std::cout
    << "\033[1m|-----------|----------|----------------------|-----------------"
       "-----|----------------------|---------------------|\033[0m\n";
...
if (is_effectively_empty(input_path)) {
  ...
  std::string status_colored =
      "\033[1;43;30m Skipped \033[0m"; // Yellow background, black text
  ...
  std::cout << "| " << std::setw(9) << test_case_number << " | " << std::setw(9)
            << status_colored << " | " << full_content(input_str) << " | "
            << full_content(result) << " | " << full_content(expected) << " | "
  ...
  if (trimmed_result == trimmed_expected) {
    status = "Passed";
    status_colored =
        "\033[1;42;30m Passed \033[0m"; // Green background, black text
  } else {
    status = "Failed";
    status_colored =
        "\033[1;41;97m Failed \033[0m"; // Red background, white text
  }
  ...
  while (std::getline(expected_stream, expected_line) &&
         std::getline(result_stream, result_line)) {
  ...
  std::cout << "| " << std::setw(9) << test_case_number << " | " << std::setw(9)
            << status_colored << " | " << std::setw(20) << compact(input_str)
            << " | " << std::setw(20) << compact(result) << " | "
            << std::setw(20) << compact(expected) << " | " << std::setw(19)
            << compact(diff_str) << " |\n";
}
...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    ...
    std::cerr << "(Set OJ=1 in environment to enable MONU_LOCAL_JUDGE macro "
                 "during compilation)\n";
    return 1;
  }
  ...
  if (fs::exists(folder / "input.txt")) {
    ...
    std::cout << "\033[1;33mNo input files found in " << folder
              << ". Only creating default files.\033[0m\n";
  }
